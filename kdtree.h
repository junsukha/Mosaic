...
class KDTree
{
  ...
  private:
    ...
    int partition(int left, int right, int pIndex, int dim);
    void quickSelect(int left, int right, int mid, int dim);
    void KDTreeHelp(int left, int right, int dim);
    KDTreeNode* treeConstruct(KDTreeNode* root, vector<Point<Dim>> v, int start, int end);
    int findNearestNeighbor(const Point<Dim> target, int left, int right, int dim) const;
